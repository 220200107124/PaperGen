...
import { Plus, BookOpen, Edit3, Trash2, Globe, GraduationCap, ChevronRight, PlusCircle, Save, CheckCircle, Loader2, AlertCircle, Sparkles } from 'lucide-react';
...
const GlobalQuestionsPage: React.FC = () => {
    ...
    const [modalData, setModalData] = useState({
        ...
        subjectId: '',
        chapterId: '',
        questionType: QuestionType.SHORT as QuestionType,
        questionText: '',
        options: ['', '', '', ''],
        answer: '0',
    ...
    useEffect(() => {
        if (filterStandard || modalStandard || modalData.standardId) {
            fetchSubjects(filterStandard || modalStandard || modalData.standardId);
        }
    }, [filterStandard, modalStandard, modalData.standardId]);

    useEffect(() => {
        if (filterSubject || modalSubject || modalData.subjectId) {
            fetchChapters(filterSubject || modalSubject || modalData.subjectId);
        }
    }, [filterSubject, modalSubject, modalData.subjectId]);
            ...
            accessor: (q: Question) => (
                <div className="flex flex-col gap-3 py-2">
                    <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-[9px] uppercase tracking-tighter">{q.questionType}</Badge>
                        <div dangerouslySetInnerHTML={{ __html: q.questionText }} className="text-lg font-bold text-gray-900 leading-relaxed font-gujarati max-w-2xl" />
                    </div>
                    {q.questionType === QuestionType.MCQ && q.options && (
                        <div className="grid grid-cols-2 gap-2 mt-2">
                            {Object.entries(q.options).map(([key, val]: any) => (
                                <div key={key} className={clsx("text-xs p-2 rounded", q.answer?.correctOption === key ? "bg-green-100 text-green-800 font-bold" : "bg-gray-50 text-gray-600")}>
                                    <span className="uppercase font-bold mr-2">{key})</span> {val}
                                </div>
                            ))}
                        </div>
                    )}
                    <div className="flex items-center gap-1 text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-2 py-1 rounded w-fit mt-1">
        ...
        },
        {
            ...
            accessor: (q: Question) => (
                <div className="flex flex-col gap-2 items-start">
                    ...
                    <span className="text-[10px] font-black text-gray-400">{q.marks} Marks</span>
                </div>
            )
        },
        {
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className="px-8 py-4 bg-white border-2 border-brand-blue text-brand-blue rounded-2xl font-black text-sm shadow-xl shadow-brand-blue/5 flex items-center gap-3 active:scale-95 transition-all hover:bg-brand-blue/5 disabled:opacity-50"
                    >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-6 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/20 items-end">
                ...
                <div className="lg:col-span-3">
                    <button
                        ...
                        className="w-full py-3.5 bg-gray-100 text-gray-400 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-200 transition-all font-bold"
                    >
                        Reset Filters
                    </button>
                </div>
            ...
            <Modal
                ...
                title={editingQuestion ? 'Update Global Question' : 'Manual Item Entry'}
                size="xl"
                footer={
                    <>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-6">
                        ...
                        {modalData.questionType === QuestionType.MCQ ? (
                            <div className="space-y-4 pt-4 border-t border-gray-100">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Multiple Choice Configuration</label>
                                <div className="grid grid-cols-2 gap-4">
                                    {['a', 'b', 'c', 'd'].map((char, i) => (
                    ...
                    <div className="space-y-6 bg-gray-100/30 p-8 rounded-[2.5rem] border border-gray-100 h-fit">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2 flex items-center gap-2">
                                <BookOpen className="w-3.5 h-3.5 text-gray-300" /> Subject
                            </label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2 flex items-center gap-2">
                                <Globe className="w-3.5 h-3.5 text-gray-300" /> Chapter
                            </label>
                        ...
                        <div className="pt-6 border-t border-gray-200 flex flex-col items-center gap-2 text-center">
                            <Badge variant="info">Global Listing</Badge>
                            <p className="text-[9px] font-bold text-gray-400 leading-relaxed uppercase tracking-widest">Publicly available for all schools</p>
                        </div>
                    </div>
                </div>
            </Modal>
